...
from project.steganalysis.torch.models import YedNet, ZhuNet, SRNet, YeNet, XuNet
from project.steganalysis.tf.models import XuNet as XuNet_tf
...
async def image_instance_level(q:Q):
    ...
    q.page['content_left'] = ui.form_card(box=ui.boxes(
        ...
        ui.text('**2. 选择模型:**'),
        ui.dropdown('framework', label='框架(单选):', value='pytorch', required=True, choices=[
            ui.choice(name=x, label=x) for x in ['pytorch', 'tensorflow']
        ]),
        ui.dropdown('source', label='数据集(单选):', value='WOW', required=True, choices=[
            ui.choice(name=x, label=x) for x in ['WOW', 'S-UNIWARD', 'HILL', 'HUGO', 'MG', 'MVG', 'UT-GAN', 'MiPOD']
        ]),
        ui.dropdown('embedding_rate', label='嵌入率(单选):', value='0.4 bpp', required=True, choices=[
            ui.choice(name=x, label=x) for x in ['0.2 bpp', '0.4 bpp', '0.6 bpp', '0.8 bpp']
        ]),
        ui.dropdown('options', label='隐写分析模型(可多选):', values=['SRNet'], required=True, choices=[
...
async def image_start_analysis(q:Q):
    ...
    models = q.args['options']
    framework = q.args['framework']
    dataset = q.args['source']
    embedding_rate = q.args['embedding_rate']
    # keep state
    q.page['content_left'].items[3].dropdown.value=framework
    q.page['content_left'].items[4].dropdown.value=dataset
    q.page['content_left'].items[5].dropdown.value=embedding_rate
    q.page['content_left'].items[6].dropdown.values=models

    ic(suspect_img_path)
    if not suspect_img_path:
    ...
    if suspect_img_path and models and framework and dataset and embedding_rate:
        def predict(suspect_img_path, models, embedding_rate, framework, dataset):
            result = {}
        ...
        await q.page.save()
        result = await q.run(predict, suspect_img_path, models, embedding_rate, framework, dataset)
        ic(result)
        ...
        q.client.suspect_img_path = None
    await q.page.save()
